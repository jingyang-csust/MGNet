...
def genConfig(seq_path, set_type):
    ...
    def get_images_from_folder(folder_path):
        return sorted([os.path.join(folder_path, p) for p in os.listdir(folder_path) if os.path.splitext(p)[1].lower() in img_extensions])

    if set_type == 'RGBT234':
        visible_path = os.path.join(seq_path, 'visible')
        infrared_path = os.path.join(seq_path, 'infrared')

        RGB_img_list = get_images_from_folder(visible_path)
        T_img_list = get_images_from_folder(infrared_path)
        ...
        RGB_gt = np.loadtxt(os.path.join(seq_path, 'visible.txt'), delimiter=',')
        T_gt = np.loadtxt(os.path.join(seq_path, 'init.txt'), delimiter=',')

    elif set_type == 'DroneT':
        rgb_path = os.path.join(seq_path, 'rgb')
        ir_path = os.path.join(seq_path, 'ir')

        RGB_img_list = get_images_from_folder(rgb_path)
        T_img_list = get_images_from_folder(ir_path)

        RGB_gt = np.loadtxt(os.path.join(seq_path, 'rgb.txt'), delimiter=',')
        T_gt = np.loadtxt(os.path.join(seq_path, 'ir.txt'), delimiter=',')

    elif set_type == 'GTOT':
        visible_path = os.path.join(seq_path, 'v')
        infrared_path = os.path.join(seq_path, 'i')

        RGB_img_list = get_images_from_folder(visible_path)
        T_img_list = get_images_from_folder(infrared_path)

        RGB_gt = np.loadtxt(os.path.join(seq_path, 'groundTruth_v.txt'), delimiter=' ')
        T_gt = np.loadtxt(os.path.join(seq_path, 'groundTruth_i.txt'), delimiter=' ')
    ...
    # 打印调试信息
    # print(f"Generated {len(RGB_img_list)} RGB images and {len(T_img_list)} T images")
    # print(f"RGB ground truth shape: {RGB_gt.shape}, T ground truth shape: {T_gt.shape}")
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter'):
    ...
    save_path = f'/root/autodl-tmp/results/{dataset_name}/' + save_name +  '/' + seq_txt + '.txt'
    save_folder = f'/root/autodl-tmp/results/{dataset_name}/' + save_name
    ...
    if len(RGB_gt) == 0:
        print(f"Error: Ground truth data is empty for sequence {seq_name}")
        return
...
if __name__ == '__main__':
    ...
    if dataset_name == 'GTOT':
        seq_home = '/root/autodl-tmp/GTOT/Multi_Modal_RGBT_dataset_CSR'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT234':
        seq_home = '/root/autodl-tmp/RGBT234/RGB_T234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT210':
        seq_home = '/root/autodl-tmp/RGBT210/RGB_T210'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home, f))]
        seq_list.sort()
        ...
        seq_home = '/root/autodl-tmp/testingset/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
