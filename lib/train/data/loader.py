...
if float(torch.__version__[:3]) >= 1.9 or len('.'.join((torch.__version__).split('.')[0:2])) > 3:
    ...
    from torch import int_classes, str, container_abcs
...
def ltr_collate_stack1(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        if _check_use_shared_memory():
            ...
            # List of expected shapes for each case
        expected_shapes = [[1, 32, 6, 128, 128], [1, 32, 4],
                           [1, 32, 6, 256, 256]]  # Adjust these to match the required shapes

        # Adjust the size of the output tensor if needed
        for expected_shape in expected_shapes:
            if out is not None and out.numel() != torch.tensor(expected_shape).prod():
                out.resize_(0)  # Resize the output tensor
        ...
        if elem.shape == ():  # scalars
            py_type = float if elem.dtype.namertswith('float') else int
            return torch.utils.data.dataloader.numpy_type_map[elem.dtype.name](list(map(py_type, batch)))
    elif isinstance(batch[0], int_classes):
        return torch.LongTensor(batch)
    elif isinstance(batch[0], float):
