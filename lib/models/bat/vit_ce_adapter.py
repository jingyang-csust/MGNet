...
from ..layers.MA import Bi_direct_adapter1
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        for i in range(depth):
            ...
            # if i == 8 or i == 9 or i == 10 or i == 11:
                blocks.append(
                    CEABlock(
                        dim=embed_dim, num_heads=num_heads, stages=i,mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
        ...
        # prompt generator #
        # self.adapter1 = Bi_direct_adapter1(embed_dim)
        # self.adapter2 = Bi_direct_adapter1(embed_dim)
        # prompt generator #
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        # prompt_x = self.adapter1(x)
        # prompt_xi = self.adapter2(xi)
        # self.x_outputs = {}  # 用字典保存每层 x 的输出
        # self.xi_outputs = {}  # 用字典保存每层 xi 的输出
        # 保存中间结果
        # self.x_outputs = {}  # 用字典保存每层 x 的输出
        # self.xi_outp/uts = {}  # 用字典保存每层 xi 的输出
        ...
        for i, blk in enumerate(self.blocks):
            # # if i == 3 or i == 6 or i == 9:
            # # 如果当前层是 CEABlock，并且之前已经有三个前层的输出
            # if i in [3, 6, 9] and len(prev_x) == 3 and len(prev_xi) == 3:
            #     # 将前三层的 x 和 xi 传递给 CEABlock
            #     x, global_index_t, global_index_s, removed_index_s, attn, \
            #         xi, global_index_ti, global_index_si, removed_index_si, attn_i = \
            #         blk(x, xi, global_index_t, global_index_ti, global_index_s, global_index_si, prev_x, prev_xi, mask_x,
            #             ce_template_mask,
            #             ce_keep_rate)
