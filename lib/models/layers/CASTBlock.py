...
from lib.models.layers.attn2 import Attention, Attention_st
...
class CEBlock(nn.Module):
    ...
    def forward(self, x, x_other, global_index_template, global_index_search, mask=None, ce_template_mask=None,
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            ...
            x, x_ori, global_index_search, removed_index_search, topk_idx = candidate_elimination(attn, x, lens_t,
                                                                                                  keep_ratio_search,
                                                                                                  global_index_search,
                                                                                                  ce_template_mask)
...
class CASTBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mode, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn_reshape = Attention_st(dim, mode, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                         proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
