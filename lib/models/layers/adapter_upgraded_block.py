...
# from lib.models.layers.bi_cga_adapt import Bi_cga_adapter
# from lib.models.layers.bi_dlk_adapt import Bi_DLK_adapter
# from lib.models.layers.mff import MFF_adapter
# from lib.models.layers.bi_mff_adapt import adapt_fusion
from lib.models.layers.adapter_upgraded import Bi_direct_adapter
# from lib.models.layers.transnext import MS_MLP
...
class CEABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads,h, w, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.mlp1 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer,
                        drop=drop)  # from timm.models.layers import Mlp, DropPath, trunc_normal_, lecun_normal_

        self.keep_ratio_search = keep_ratio_search

        self.adap_mff = Bi_direct_adapter()
        self.adap2_mff = Bi_direct_adapter()

    def forward(self, x, xi, lens_z, global_index_template, global_index_templatei, global_index_search,
                global_index_searchi, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        x = x + self.drop_path(self.mlp1(self.norm4(x))) + self.drop_path(
            self.adap2_mff(self.norm4(xi)))  ###-------adapter

        xi = xi + self.drop_path(self.mlp1(self.norm4(xi))) + self.drop_path(
            self.adap2_mff(self.norm4(xori)))  ###-------adapter
