...
class fuision(nn.Module):
    ...
    def forward(self,x_v,x_i,lens_z):
        ...
        x_fusion = xo + + x_i
        # xo = patch2token(xo)
        x_v = x_fusion + self.mlp1(token2patch(self.norm(patch2token(x_fusion))))
        x_v = patch2token(x_v)
        ...
        z_fusion = zo1 + zo2 + z_i
        # zo = zo.flatten(2).transpose(1, 2).contiguous()  # ([2, 256, 1536])
        z_fusion = z_fusion + self.mlp2(token2patch(self.norm(patch2token(z_fusion))))
        z_v = patch2token(z_fusion)
