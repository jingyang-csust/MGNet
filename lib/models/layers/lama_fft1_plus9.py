...
#         self.conv1x1_real = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0)
#         self.conv1x1_imag = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0)
#         self.act = nn.ReLU()
#
#
#     def forward(self, x):
...
#         x_down_x = x_down[:,64:,:]
#
#         x_down_z = x_down_z + self.attn1(self.norm(x_down_z),self.norm(x_down_x))
#         x_down_x = x_down_x + self.attn2(self.norm(x_down_x),self.norm(x_down_z))
#         x_down_z = self.norm(x_down_z)
#         x_down_x = self.norm(x_down_x)
#         # tmp = x_down_z + x_down_x
#         # x_down_z = self.norm(tmp + self.attn3(x_down_x,x_down_z))
#         # x_down_z = x_down_z_raw + self.mlp_z(x_down_z_new)
#         # x_down_x = self.norm(x_down_x + self.fnn3(x_down_x))
#
#
#         x_down = torch.cat((x_down_z,x_down_x),dim=1)
#         # x_down = x_down + self.attn3(self.norm(x_down))
#         # x_down = self.norm(x_down + self.fnn3(x_down))
#         # print(x_down.shape)
...
from lib.models.layers.bi_dlk_adapt import DropPath
from lib.utils.token_utils import token2patch,patch2token
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x1, x2, mask=None):
        ...
        attn = F.softmax(attn, dim=-1)
        output = torch.matmul(attn, v2).permute(0, 2, 1, 3).contiguous().view(batch_size, seq_len1, -1)
        output = res + self.proj_o(output)
        output = self.norm(output)
        output = token2patch(output)
