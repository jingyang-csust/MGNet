...
from lib.models.layers.hip import SpatialGate, ChannelGate
...
class fuision(nn.Module):
    ...
    def __init__(self,dim):
        super().__init__()
        self.channels = dim
        self.block1_x = SpatialGate()
        # self.linear_x = nn.Linear(dim * 2, self.channels)
        self.block2_x = ChannelGate(self.channels)
        # self.block3_x = LSA(self.channels,self.channels)
        # self.block4_x = ShuffleAttention(self.channels)

        self.block1_z = SpatialGate()
        # self.linear_z = nn.Linear(dim * 2, self.channels)
        self.block2_z = ChannelGate(self.channels)
        # self.block3_z = LSA(self.channels,self.channels)
        # self.block4_z = ShuffleAttention(self.channels)
    ...
    def forward(self,x):
        ...
        x1 = self.block1_x(x_v)
        # x = patch2token(x)
        xo = self.block2_x(x1)
        # x3 = patch2token(x)
        # x3 = self.block3_x(x3,16,16)
        # x3 = token2patch(x3)
        # x2 = token2patch(x2)
        # x4 = self.block4_x(x)
        # xo_tmp = x1 + x2 +x3 + x4
        ...
        z1 = self.block1_z(z_v)
        # z = patch2token(z)
        zo = self.block2_z(z1)
        # z3 = patch2token(z)
        # z3 = self.block3_z(z3,8,8)
        # z3 = token2patch(z3)
        # z4 = self.block4_z(z)
        # z2 = token2patch(z2)
        # zo_tmp = z1 + z2 + z3 + z4
