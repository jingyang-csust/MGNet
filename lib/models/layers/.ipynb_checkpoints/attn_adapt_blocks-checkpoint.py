...
from lib.models.layers.FFT import SGBlock
from lib.models.layers.attn import Attention
from lib.models.layers.adapter import Bi_direct_adapter
from lib.models.layers.dw_adapter import MS_Fusion
from lib.models.layers.MS_MD_Fusion import MS_MD_Fusion
from lib.models.layers.FFT_P import SGBPBlock
from lib.models.layers.DWT_Enhance import DWTBlock
...
class CEABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, channel, control_fft, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, keep_ratio_search=1.0,):
        super().__init__()
        mlp_hidden_dim = int(dim * mlp_ratio)
        # print(channel) 320
        self.mlp0 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        ...
        self.mlp1 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)     #from timm.models.layers import Mlp, DropPath, trunc_normal_, lecun_normal_
    ...
    def forward(self, x, xi, lens_x, global_index_template, global_index_templatei, global_index_search, global_index_searchi, mask=None, ce_template_mask=None, keep_ratio_search=None):
        # print(f"进入bat之前x的形状：{x.shape}")  ([2, 320, 768])
        # print(f"self.h：{self.h},self.w{self.w}")        
        xori = x

        x_attn, attn = self.attn(self.norm3(x), mask, True)
        x = x + self.drop_path(x_attn) + self.drop_path(
            self.adap_t(self.norm3(xi)))  #########-------------------------adapter

        xi_attn, i_attn = self.attn(self.norm3(xi), mask, True)
        xi = xi + self.drop_path(xi_attn) + self.drop_path(
            self.adap_t(self.norm3(xori)))  #########-------------------------adapter
        ...
        x = x + self.drop_path(self.mlp1(self.norm4(x))) + self.drop_path(
            self.adap2_t(self.norm4(xi)))  ###-------adapter

        xi = xi + self.drop_path(self.mlp1(self.norm4(xi))) + self.drop_path(
            self.adap2_t(self.norm4(xori)))  ###-------adapter
