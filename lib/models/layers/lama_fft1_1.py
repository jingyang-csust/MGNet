...
class SpectralGatingNetwork(nn.Module):
    def __init__(self, dim, h=20, w=20, SC=768, drop=0.):
        ...
        # self.conv1x1_real_2 = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0)
        self.conv1x1_imag = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0)
        # self.conv1x1_imag_2 = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0)
        self.act = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x_fft_imag = self.conv1x1_real(x_fft_imag)
        x_fft_real = self.act(x_fft_real)
        x_fft_imag = self.act(x_fft_imag)
        x_fft_real = self.conv1x1_imag(x_fft_real)
...
#         self.stb_x1 = SGBlock(dim)
#         self.stb_x2 = SGBlock(dim)
#
