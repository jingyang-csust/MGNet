...
from lib.models.layers.MA import SE
from lib.utils.token_utils import token2patch, patch2token
from model.encoders.vmamba import ECAAttention, LSA
...
from model.encoders.selective_scan.selective_scan.selective_scan_interface import selective_scan_fn as selective_scan_fn_v1
...
#         xo = self.fc2(F.gelu(self.pwconv2(raw + x + x1 + x2, H, W)))
#         xo = token2patch(xo)
#         return xo
class CrossMambaFusion_SS2D_SSM(nn.Module):
    ...
    def __init__(
        ...
        if self.d_conv > 1:
            ...
            self.act = nn.SiLU()
        # self.attn1 = SpatialGroupEnhance()
        # self.attn2 = SpatialGroupEnhance()
        ...
        self.attn1 = ECAAttention()
        self.attn2 = ECAAttention()
        self.out_proj = nn.Linear(self.d_model*2, self.d_model, bias=bias, **factory_kwargs)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
        self.se = SE(self.d_model)

    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        x_rgb1 = self.in_proj(x_rgb)
        x_e1 = self.in_proj_modalx(x_e)
        B, H, W, D = x_rgb1.shape
        if self.d_conv > 1:
            # x_rgb = self.attn(x_rgb)
            # x_e = self.attn(x_e)
            x_rgb_trans = x_rgb1.permute(0, 3, 1, 2).contiguous()
            x_e_trans = x_e1.permute(0, 3, 1, 2).contiguous()
            x_rgb_conv = self.act(self.conv2d(x_rgb_trans))  # (b, d, h, w)
            x_e_conv = self.act(self.conv2d(x_e_trans))  # (b, d, h, w)
            # x_rgb_conv = self.attn(x_rgb_conv)
            # x_e_conv = self.attn(x_e_conv)
        ...
        out_rgb = x_rgb + self.dropout_rgb(self.out_proj_rgb(y_rgb))
        out_e = x_e + self.dropout_e(self.out_proj_e(y_e))
        y = torch.concat([out_rgb, out_e], dim=-1)
        # print(y.shape)
        out = self.dropout(self.out_proj(y))
        # print(out.shape)
        out = self.se(out.permute(0,3,1,2).contiguous()).permute(0, 2, 3, 1).contiguous()

        # out_rgb = self.attn1(out_rgb)
        # out_e = self.attn2(out_e)
        return out
...
class CrossMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        sum1 = self.drop_path1(self.op(x_rgb, x_e))
        sum2 = self.drop_path1(self.op(x_e, x_rgb))
        # x_rgb = x_rgb + self.drop_path1(x_rgb_cross)
        # x_e = x_e + self.drop_path2(x_e_cross)
        if self.state == 1:
            x_fused = self.fuse(sum1.permute(0,3,1,2) + sum2.permute(0,3,1,2))
            # print("------")
            return x_fused
