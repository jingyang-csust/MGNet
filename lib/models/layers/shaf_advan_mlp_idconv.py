...
from lib.models.layers.dwconv import MultiScaleDWConv
from lib.models.layers.fudion8 import DynamicConv2d
...
class SHA_Fusion(nn.Module):
    ...
    def __init__(self,dim):
        ...
        self.conv_x = DynamicConv2d(self.channels,num_groups=2)
        self.linear = nn.Linear(dim * 2, self.channels)

        self.sha_z = ShuffleAttention(dim)
        self.conv_z = DynamicConv2d(self.channels,num_groups=2)
        self.linear_z = nn.Linear(dim * 2, self.channels)

        self.mlp1 = Mlp(in_features=self.channels, hidden_features=self.channels * 2, h=16, w=16)
        self.norm = nn.LayerNorm(self.channels)
        self.mlp1_z = Mlp(in_features=self.channels, hidden_features=self.channels * 2, h=8, w=8)
    ...
    def forward(self,x_v,x_i,lens_z):
        ...
        x1 = self.sha(x)  # ([2, 768, 16, 16])
        x2 = self.conv_x(x)
        x = x1 + x2
        x_v1 = x * x_v  # +
        x_i1 = x * x_i  # +
        x = x_v1 + x_i1 + x_v  # *
        x = patch2token(x) + self.norm(patch2token(self.mlp1(x)))
        ...
        z1 = self.sha_z(z)  # ([2, 768, 16, 16])
        z2 = self.conv_x(z)
        z = z1 + z2
        z_v1 = z * z_v  # +
        z_i1 = z * z_i  # +
        z = z_v1 + z_i1 + z_v  # *
        z = patch2token(z) + self.norm_z(patch2token(self.mlp1_z(z)))
