...
# from lib.models.layers.MA import SE, CBAMLayer, GAM_Attention
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.d_conv > 1:
            ...
            #     **factory_kwargs,
            # )
            # self.conv2d1 = MS_FFN(self.d_inner)
            # self.conv2d2 = MS_FFN(self.d_inner)
    ...
    def forward(self, x: torch.Tensor):
        xz = self.in_proj(x)
        # xzi = self.in_proj(x_i)

        xz = token2patch(xz).permute(0,2,3,1).contiguous()
        # xzi = token2patch(xzi).permute(0,2,3,1).contiguous()

        if self.d_conv > 1:
            x, z = xz.chunk(2, dim=-1)  # (b, h, w, d)
            # xi, zi = xzi.chunk(2, dim=-1)  # (b, h, w, d)
            x = x.permute(0, 3, 1, 2).contiguous()
            # xi = xi.permute(0, 3, 1, 2).contiguous()
            x = self.act(self.conv2d(x))  # (b, d, h, w)
            # xi = self.act(self.conv2d(xi))  # (b, d, h, w)
            # temp = x * xi + x + xi
        ...
        out = patch2token(out.permute(0,3,1,2).contiguous())
        return out
...
class ConMB_SS2D(nn.Module):
    ...
    def     __init__(
        ...
        **kwargs,
        ...
        # self.attn3 = CFN(self.d_model,self.d_model)
        self.attn4 = ECAAttention()
        ...
        # self.attn_sum = GAM_Attention(self.d_model,self.d_model)
    @staticmethod
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4, **factory_kwargs):
        dt_proj = nn.Linear(dt_rank, d_inner, bias=True, **factory_kwargs)
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        # print(out.shape)
        # out = self.attn4(out.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        return out
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input1):
        # print(input.shape)
        x = self.drop_path(self.op(self.norm(input1)))
        if self.mlp_branch:
    ...
    def forward(self, input1):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input1)
        else:
            return self._forward(input1)
