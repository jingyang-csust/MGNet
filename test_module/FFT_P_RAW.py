...
class SGBlock(nn.Module):
    ...
    def forward(self, x, lens_x):
        ...
        # res_x = res_x.view(B, spatial_size, spatial_size, C).permute(0, 3, 1, 2)
        # x1 = self.act(self.conv3x3(res_x))
        # x2 = self.act(self.conv1x1(res_x))
        # x_p = x1 + x2
        # print(f"pre_x_p:{x_p.shape}")
        #
        # x_p = x_p.permute(0, 2, 3, 1).reshape(B, n, C)
        # print(f"x_p:{x_p.shape}")
        # x_p = torch.cat([res_z, x_p], dim=1)
        # print(f"later_x_p:{x_p.shape}")
        x = self.act(x_p + x0)
