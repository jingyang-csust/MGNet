...
class Mlp(nn.Module):
    ...
    def forward(self, x, lens_x):
        ...
        template = x[:, :lens_x]
        search = x[:, lens_x:]
        # print(template.shape) Expected output shape: [1, 64, 768]
        # print(search.shape) Expected output shape: [1, 256, 768]
        # 处理搜索区域
        search = search.view(batch_size * (num_patches - lens_x), in_features)
